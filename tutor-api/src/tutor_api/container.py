...
Providers live here rather than as decorators on the classes themselves, so
``settings.py``, the adapters and the domain stay free of any DI import. A
class that reaches for its own container is not injected, it is coupled, and
the capability-scoping argument in DEC-0001 rests on a collaborator holding
...
class DatabaseSettingsProvider(Provider):
    ...
    A singleton because configuration is process-wide and immutable, and
    because it holds no learner data — the two questions the lifetime check
